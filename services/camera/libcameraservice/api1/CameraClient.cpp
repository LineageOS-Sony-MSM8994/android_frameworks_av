...
CameraClient::CameraClient(const sp<CameraService>& cameraService,
        ...
        Client(cameraService, cameraClient, attributionAndPermissionUtils, clientAttribution,
                ...
                api1CameraId, cameraFacing, sensorOrientation, servicePid, compatInfo, sharedMode),
...
status_t CameraClient::setAudioRestriction(
        hardware::camera2::ICameraDeviceUser::AudioRestriction mode) {
    if (!isValidAudioRestriction(mode)) {
        ALOGE("%s: invalid audio restriction mode %d", __FUNCTION__, static_cast<int32_t>(mode));
        return BAD_VALUE;
    }
...
hardware::camera2::ICameraDeviceUser::AudioRestriction CameraClient::getGlobalAudioRestriction() {
    Mutex::Autolock lock(mLock);
    if (checkPidAndHardware() != NO_ERROR) {
